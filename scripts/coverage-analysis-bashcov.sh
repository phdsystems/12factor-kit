...
# --skip-uncovered doesn't report files with 0% coverage
if bashcov --root . "$TEST_SCRIPT" 2>&1 | tail -20; then
    echo
...
if [[ -d "$COVERAGE_DIR" ]]; then
    ...
    echo -e "${YELLOW}This might be due to the test hanging. Try running with timeout:${NC}"
    echo -e "  ${CYAN}timeout 30 bashcov --root . $TEST_SCRIPT${NC}"
    exit 1
fi
